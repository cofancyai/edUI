...
- Maintain academic quality in the selected language    systemPrompt = `You are an expert educational content creator specializing in comprehensive theoretical analysis. Create detailed academic content with deep analysis, theoretical foundations, and scholarly depth.
...
- Generate all content in ${language} language
- Use proper grammar and terminology for ${language}
- Maintain academic quality in the selected language  
  return { systemPrompt, userPrompt };
};

// AI-powered color coding function
const addIntelligentColorCoding = async (content) => {
  if (!content || content.length < 10) return content;

  try {
    const prompt = `Analyze this text and add HTML color coding for different entity types.

Text: "${content}"

Add <span> tags with these colors:
- Places/Countries/Cities: style="color: #06B6D4; font-weight: 600; background: rgba(6, 182, 212, 0.1); padding: 1px 4px; border-radius: 3px;"
- People/Names: style="color: #EC4899; font-weight: 600; background: rgba(236, 72, 153, 0.1); padding: 1px 4px; border-radius: 3px;"
- Years/Dates: style="color: #10B981; font-weight: 600; background: rgba(16, 185, 129, 0.1); padding: 1px 4px; border-radius: 3px;"
- Historical Events/Wars/Treaties: style="color: #8B5CF6; font-weight: 600; background: rgba(139, 92, 246, 0.1); padding: 1px 4px; border-radius: 3px;"
- Money/Numbers/Measurements: style="color: #F59E0B; font-weight: 600; background: rgba(245, 158, 11, 0.1); padding: 1px 4px; border-radius: 3px;"

Only return the enhanced HTML text with color coding. Do not add explanations.`;
    ...
    const response = await axios.post(TOGETHER_API_URL, {
      ...
      messages: [
        {
          role: "system",
          content: "You are an expert at identifying and color-coding different types of entities in text. Return only the HTML-enhanced text."
        },
        {
          role: "user",
          content: prompt
    ...
    const colorCodedContent = response.data.choices[0].message.content.trim();
    return colorCodedContent || content;
  } catch (error) {
    console.error("Color coding error:", error);
    return content;
  }
};
...
// AI-powered color coding endpoint
router.post("/analyze-content", async (req, res) => {
  ...
  try {
    ...
    if (query === "color_coding") {
      const prompt = `Analyze this text and add HTML color coding:
- Years/dates: <span style="color: #F59E0B; font-weight: 600;">YEAR</span>
- Places/names: <span style="color: #3B82F6; font-weight: 600;">NAME</span>
- Scientific terms: <span style="color: #10B981; font-weight: 600;">TERM</span>
- Technical concepts: <span style="color: #8B5CF6; font-weight: 600;">CONCEPT</span>

Text to color: ${content}`;
      
      const coloredContent = await callAIForColoring(prompt);
      res.json({ coloredContent });
    }
  } catch (error) {
